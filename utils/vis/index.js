...
function draw(data, type) {
	if (!!window) {
		...
		d3.select(".viz > *").remove(); // Clear the 'canvas' first
		...
		data.forEach(function(o) {
			o.dayFromZero = o.record_date.diff(
				patientZeroDate,
				"days"
			);
		});
		...
		d3.select(`.viz.${class_name}`).attr(
			"id",
			`svg-viz-${class_name}`
		); // Assign id to element ; svg-viz
		...
		})(); // Reset width is necessary (if the client is a small screen)
		...
		var svg = d3
			...
			.append("g")
			.attr("class", "graph-container"); // Append an svg element inside the container
